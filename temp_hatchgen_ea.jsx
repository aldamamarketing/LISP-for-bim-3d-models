...
export default function HatchGenerator({ lang = 'en' }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error(error);
      showToast("Instabilidade nos servi├ºos de IA. Tente novamente em instantes.", "error");
    } finally {
  ...
  const handleSaveFavorites = async () => {
    if (!user) {
      showToast(
        'Crie uma conta gratuita para salvar nos favoritos. <a href="/login?redirect=' + encodeURIComponent(window.location.pathname) + '" style="color:#f26d21;font-weight:bold;text-decoration:underline">Criar conta ÔåÆ</a>',
        'warning',
        6000
      );
      return;
    ...
    try {
      for (const hatch of selectedHatches) {
        // Generados por IA puede que no est├®n en BD p├║blica, guardarlos primero.
      ...
      showToast('Adicionados aos Favoritos com sucesso! Ô¡É', 'success');
      setSelectedHatches([]); // Limpiar selecci├│n tras a├▒adir? o mantener.
    } catch (error) {
  ...
  const toggleSelectHatch = (hatch) => {
    ...
    if (isSelected) {
      ...
      // Normalizar para que library items y result items tengan misma estructura b├ísica
  ...
  const handleExport = () => {
    ...
    showToast(
      '­ƒôÑ Download conclu├¡do! <strong>Dica:</strong> Com o LispCentral Loader, voc├¬ usa esses hatches direto no AutoCAD sem baixar arquivos.',
      'info',
      8000
    );
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', width: '100%', gap: '20px' }}>
              ...
              <div className="panel-body">
                <div className="form-group">
                  ...
                  <select className="form-control" value={theme} onChange={(e) => setTheme(e.target.value)}>
                    ...
                    <option value="Topograf├¡a">Topografia (Terrenos)</option>
                    <option value="Materiales">Materiais (A├ºo, Madeira)</option>
                  </select>
                </div>
            ...
            <div style={{ flex: 1, backgroundColor: '#222', borderRadius: '8px', padding: '15px', display: 'flex', flexDirection: 'column', height: '100%' }}>
              ...
              <div style={{ 
                ...
                {isGenerating && <p style={{ color: 'var(--tmd-orange)' }}>Gerando padr├Áes, aguarde...</p>}
                ...
                {!isGenerating && results.length === 0 && (
                  <p style={{ color: 'var(--text-muted)' }}>Escreva os comandos ├á esquerda e clique em Gerar.</p>
                )}
              </div>
            </div>
          </div>
      ...
      {selectedHatches.length > 0 && (
        <div style={{
          ...
          >
            Ô¡É {saving === 'all' ? 'Salvando...' : 'Adicionar ├á Paleta'}
          </button>
          ...
          >
            Ô¼ç´©Å {isExporting ? 'Processando...' : 'Baixar .PAT'}
          </button>
        </div>
      )}
