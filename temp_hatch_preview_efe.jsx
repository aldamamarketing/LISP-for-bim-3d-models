...
export default function HatchPreview({ patCode, scale = 1 }) {
  ...
  useEffect(() => {
    ...
    // Suficiente para cubrir el ├írea
    const maxDist = Math.max(width, height) * 3; 

    patterns.forEach(pat => {
      const angleRad = pat.angle * Math.PI / 180;
      
      // Preparar guiones. En AutoCAD: + es l├¡nea, - es espacio, 0 es punto.
      // Math.abs convierte los espacios negativos en distancias de canvas v├ílidas.
      // Canvas ignora el dash 0, as├¡ que lo convertimos a un valor muy peque├▒o (0.5) para que sea un punto.
      const canvasDashes = pat.dashes.map(d => d === 0 ? 0.5 : Math.abs(d) * scale * 10); // Multiplicamos x10 para que los patrones peque├▒os se vean
      
      ctx.save();
      // Origen en el centro para facilitar la visualizaci├│n
      ...
      // Si dy es 0, dibujamos una sola l├¡nea en el centro
      if (Math.abs(dy) < 0.001) {
        ...
        for (let i = -numLines; i <= numLines; i++) {
          ...
          // Desfase del dash para que el patr├│n se escalone correctamente
