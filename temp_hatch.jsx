...
export default function HatchGenerator({ lang = 'en' }) {
  ...
  // Estados de b├║squeda para arquetipos
  ...
  useEffect(() => {
    // Cuando cambia el arquetipo, reiniciamos los par├ímetros
  ...
  // Forzar relaci├│n l├│gica entre dimensiones para patrones espec├¡ficos
  useEffect(() => {
    if (currentArchetype.id === 'chevron') {
      // Chevron: El Largo (width) no puede ser menor que el Ancho (height).
      // Matem├íticamente un Chevron requiere que la tabla sea m├ís larga que ancha.
      if (width < height) {
        setWidth(height);
      }
    }
  ...
  const handleSaveToFavorites = async () => {
    ...
    try {
      ...
      await addToFavorites(assetData); // Guarda el c├│digo y datos completos en la colecci├│n privada
      showToast('Padr├úo salvo na sua cole├º├úo! Ô¡É', 'success');
    } catch (error) {
  ...
  const handleSaveToPublicLibrary = async () => {
    ...
    try {
      ...
      await addToFavorites(assetData); // Tambi├®n a favoritos del admin para acceso r├ípido
      showToast('Padr├úo publicado na Livraria Global! ­ƒîì', 'success');
    } catch (error) {
      showToast(error.message, 'error');
    }
    setSaving(false);
  ...
  return (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative' }}>
      ...
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', gap: '20px' }}>
        ...
        {/* Pesta├▒a: Biblioteca (Para ver y gestionar los Favoritos) */}
        {activeTab === 'library' && (
          <LibraryPanel 
            currentType="hatch" 
        ...
        {/* Pesta├▒a: Constructor Param├®trico */}
        {activeTab === 'generator' && generatorView === 'archetypes' && (
          <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', backgroundColor: '#0f172a', borderRadius: '8px', border: '1px solid #334155', overflow: 'hidden' }}>
            <div style={{ padding: '20px', borderBottom: '1px solid #334155', display: 'flex', gap: '15px', alignItems: 'center', backgroundColor: '#1e293b' }}>
              <h2 style={{ margin: 0, color: 'var(--tmd-orange)', flexShrink: 0 }}>Arquetipos Param├®tricos</h2>
              <input type="text" placeholder="Buscar patr├│n..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ flex: 1, padding: '10px 15px', borderRadius: '6px', border: '1px solid #475569', backgroundColor: '#0b0f19', color: 'white' }} />
          ...
          <div style={{ display: 'flex', width: '100%', gap: '20px' }}>
            ...
            <div className="panel col-settings" style={{ width: '320px', flexShrink: 0, overflowY: 'auto' }}>
              <div style={{ padding: '15px', borderBottom: '1px solid #333', backgroundColor: '#1a1a1a', borderTopLeftRadius: '8px', borderTopRightRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button onClick={() => setGeneratorView('archetypes')} style={{ background: 'none', border: 'none', color: 'var(--tmd-orange)', cursor: 'pointer', fontSize: '0.8rem', fontWeight: 'bold' }}>Ô¼à Volver</button>
              </div>
