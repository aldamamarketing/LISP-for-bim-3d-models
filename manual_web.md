...
2. **Plantilla Blob:** El código fuente completo de `TMD_Loader.lsp` está definido en un `template literal` en JavaScript.
3. **Inyección:** La API Key generada se inyecta dinámicamente en la variable `*LISPCENTRAL-KEY*` dentro del template.
4. **Descarga Instantánea:** Se crea un objeto `Blob` tipo `text/plain` y se fuerza la descarga mediante la creación y clic virtual de un tag `<a>`. 
