...
export default function LibraryPanel({
  ...
  const handleSave = async (assetId) => {
    if (!user) {
      showToast(
        'Crie uma conta gratuita para salvar nos favoritos e usar direto no AutoCAD. <a href="/login?redirect=' +
          encodeURIComponent(window.location.pathname) +
          '" style="color:#f26d21;font-weight:bold;text-decoration:underline">Criar conta →</a>',
        "warning",
        6000,
      );
      return;
  ...
  const filtered = useMemo(() => {
    return assets.filter((a) => {
      if (activeCategory !== "Todas" && a.category !== activeCategory)
        return false;
      if (searchQuery && searchQuery.trim().length > 2) {
        ...
        if (keywords.length > 0) {
          const textToSearch =
            `${a.name || ""} ${a.description || ""}`.toLowerCase();
    ...
    >
      <div style={{ marginBottom: "15px" }}>
        <h3 style={{ margin: 0, color: "#fff", fontSize: "1.1rem" }}>
          Biblioteca Pública ({filtered.length})
        </h3>
        <p
          style={{
            margin: "5px 0 0 0",
        ...
        {categories.map((cat) => (
          <button
            ...
            style={{
              padding: "6px 12px",
              backgroundColor:
                activeCategory === cat ? "var(--tmd-orange)" : "#333",
              color: "#fff",
              border: "none",
              borderRadius: "15px",
              cursor: "pointer",
