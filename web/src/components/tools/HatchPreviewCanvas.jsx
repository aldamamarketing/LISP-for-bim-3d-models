...
export default function HatchPreviewCanvas({ archetype, width, height, joint, rows, columns }) {
  ...
  // Tamaño del área central definida por el usuario
  const coreWidth = cellW * columns;
  const coreHeight = cellH * rows;
  
  // El lienzo físico completo, dándole un margen de sobrepaso para mostrar el patrón "continuo"
  ...
  // Renderizador específico por arquetipo
    ...
    for (let r = 0; r < totalRows; r++) {
      for (let c = 0; c < totalCols; c++) {
        ...
        if (archetype === 'stack' || archetype === 'cubic') {
          ...
          // pero el patrón Architextures clásico tiene 3 rectangulos verticales, 3 horizontales.
          ...
          if (cycleRow < 5) {
            ...
            // Necesitamos dibujar el doble de headers para cubrir la misma longitud (c cubre hasta totalCols, así que basta con dibujar 2)
          ...
          // Simplificación: Un rombo/rectángulo rotado
          const angle = archetype === 'herringbone' ? 45 : 30; // Herringbone = 45 deg
          const color = "#a8a8a8";
          // Como dibujar los SVGs perfectos de herringbone requiere matemáticas extensas,
          // usaremos un polígono simplificado que denote la dirección.
  ...
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ marginTop: '10px', fontSize: '0.8rem', color: '#888' }}>
        Preview: {coreWidth} x {coreHeight} mm (Área seleccionada en naranja)
      </div>
    </div>
  );
}
