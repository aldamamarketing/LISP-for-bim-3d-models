...
export default function LispFilesCard() {
  ...
  const toggleExpand = (id) => {
    if (expandedFileIds.includes(id)) setExpandedFileIds(expandedFileIds.filter(i => i !== id));
  ...
  const handleCommandUpdate = async (cmdId, field, value) => {
    try {
      ...
      setCommands(commands.map(c => c.id === cmdId ? { ...c, [field]: value } : c));
    } catch (e) {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        if (file.type === 'image/svg+xml') {
          ...
          let sanitized = result.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '');
          // Make sure width/height is flexible
          sanitized = sanitized.replace(/width="[^"]*"/, 'width="100%"').replace(/height="[^"]*"/, 'height="100%"');
          resolve(sanitized);
    ...
    <div className="tab-enter card pb-32">
      ...
      {draftFiles.length > 0 && (
        <div className="mb-8 p-4 border border-primary-container/30 bg-primary-container/5 rounded-lg">
          <h4 className="text-primary-container text-sm font-bold mb-3">Revisão de Upload ({draftFiles.length})</h4>
          <div className="space-y-3">
            {draftFiles.map((draft, i) => (
              <div key={i} className="flex items-start justify-between bg-surface p-3 rounded border border-outline-variant">
                <div>
                  <div className="font-code-sm text-on-surface font-bold">{draft.originalName}</div>
                  <div className="text-xs text-on-surface-variant mt-1">
                    Comandos detectados: {draft.detectedCommands.map(c => c.commandName).join(', ') || 'Nenhum'}
                  </div>
                </div>
                <button className="text-error hover:text-white focus-visible:ring-2 focus-visible:ring-error outline-none rounded" aria-label="Excluir rascunho" title="Excluir rascunho" onClick={() => removeDraft(i)}>
          ...
          <div className="mt-4 flex justify-end">
            <button className="btn bg-primary-container text-white text-sm" onClick={submitAllDrafts} disabled={isUploading}>
      ...
      {/* DENSE TABLE */}
      <div className={`overflow-x-auto border border-outline-variant ${selectedIds.length > 0 ? 'rounded-b-lg border-t-0' : 'rounded-lg'} bg-surface`}>
          ...
          <thead>
            <tr className="border-b border-outline-variant text-label-md font-label-md text-on-secondary-container bg-surface-container-low">
              <th className="py-2 pl-4 font-normal w-[40px]">
                <input type="checkbox" onChange={toggleSelectAll} checked={tenantLisps.length > 0 && selectedIds.length === tenantLisps.length} className="rounded border-outline-variant bg-surface checked:bg-primary-container" />
              </th>
              <th className="py-2 font-normal">Arquivo LISP / Comandos Extraídos</th>
              <th className="py-2 font-normal w-[200px]">Data Upload</th>
            </tr>
          </thead>
          <tbody>
            {tenantLisps.map(lisp => {
              const fileCommands = commands.filter(c => c.lispFileId === lisp.id);
              const isExpanded = expandedFileIds.includes(lisp.id);

              return (
                <React.Fragment key={lisp.id}>
                  {/* PARENT ROW (FILE) */}
                  <tr className="border-b border-outline-variant hover:bg-surface-container-high transition-colors cursor-pointer" onClick={() => toggleExpand(lisp.id)}>
                    <td className="py-3 pl-4" onClick={(e) => e.stopPropagation()}>
                      <input type="checkbox" checked={selectedIds.includes(lisp.id)} onChange={() => toggleSelect(lisp.id)} className="rounded border-outline-variant bg-surface checked:bg-primary-container" />
                    </td>
                    <td className="py-3 text-on-surface">
                      <div className="flex items-center gap-2">
                        <span className={`material-symbols-outlined text-[18px] text-on-surface-variant transition-transform ${isExpanded ? 'rotate-90' : ''}`}>chevron_right</span>
                        <span className="material-symbols-outlined text-[18px] text-secondary">description</span>
                        <span className="font-code-sm font-bold text-sm">{lisp.originalName}</span>
                              ...
                              >
                                {cmd.svgIcon ? (
                                  cmd.svgIcon.startsWith('data:image') 
                                    ? <img src={cmd.svgIcon} alt="icon" className="w-full h-full object-contain p-1 rounded" />
                                    : <div dangerouslySetInnerHTML={{ __html: cmd.svgIcon }} className="w-5 h-5 flex items-center justify-center [&>svg]:w-full [&>svg]:h-full" />
                                ) : (
                                  <span className="material-symbols-outlined text-[16px] opacity-50">image</span>
                                )}
                              </button>
                              ...
                              {dropdownOpenFor === cmd.id && (
                                <div className="absolute top-10 left-0 w-48 bg-surface border border-outline-variant rounded-lg shadow-xl z-50 py-1">
                                  <label className="flex items-center gap-2 px-4 py-2 hover:bg-surface-container cursor-pointer text-sm text-on-surface-variant hover:text-on-surface">
                                    <span className="material-symbols-outlined text-[16px]">upload</span> Subir Arquivo (SVG/PNG/JPG)
                                    <input type="file" accept=".svg,.png,.jpg,.jpeg" className="hidden" onChange={(e) => handleIconUpload(e, cmd.id)} />
                                  </label>
                                  <button className="w-full flex items-center gap-2 px-4 py-2 hover:bg-surface-container text-left text-sm text-on-surface-variant hover:text-on-surface" onClick={() => { setActiveCommandId(cmd.id); setShowFavoritesModal(true); setDropdownOpenFor(null); }}>
                                    <span className="material-symbols-outlined text-[16px]">star</span> Meus Favoritos
                                  </button>
                                  <button className="w-full flex items-center gap-2 px-4 py-2 hover:bg-surface-container text-left text-sm text-on-surface-variant hover:text-on-surface" onClick={() => { setActiveCommandId(cmd.id); setShowGalleryModal(true); setDropdownOpenFor(null); }}>
                                    <span className="material-symbols-outlined text-[16px]">storefront</span> Galeria Pública
                                  </button>
                                </div>
                              )}
                            </div>
            ...
            {tenantLisps.length === 0 && (
              <tr>
                <td colSpan="3" className="py-8 text-center text-on-surface-variant">
                  Nenhum arquivo LISP carregado. Arraste arquivos aqui ou clique no botão acima.
                </td>
              </tr>
            )}
          </tbody>
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4">
          <div className="bg-[#141414] border border-[#262626] rounded-xl w-full max-w-[600px] p-6 shadow-2xl">
            <h3 className="mt-0 mb-4 text-xl flex justify-between items-center">
              ...
              <button className="text-on-surface-variant hover:text-white focus-visible:ring-2 focus-visible:ring-primary-container outline-none rounded" aria-label="Fechar" title="Fechar" onClick={() => setShowGalleryModal(false)}><span className="material-symbols-outlined text-[20px]">close</span></button>
            </h3>
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4">
          <div className="bg-[#141414] border border-[#262626] rounded-xl w-full max-w-[600px] p-6 shadow-2xl">
            <h3 className="mt-0 mb-4 text-xl flex justify-between items-center">
              ...
              <button className="text-on-surface-variant hover:text-white focus-visible:ring-2 focus-visible:ring-primary-container outline-none rounded" aria-label="Fechar" title="Fechar" onClick={() => setShowFavoritesModal(false)}><span className="material-symbols-outlined text-[20px]">close</span></button>
            </h3>
            <div className="h-64 flex items-center justify-center border border-dashed border-[#262626] rounded text-on-surface-variant text-sm">
