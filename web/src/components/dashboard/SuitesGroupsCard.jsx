...
export default function SuitesGroupsCard() {
  ...
  const handleSaveSuite = async () => {
    ...
    try {
      ...
      if (editSuiteData.isNew) setSuites([...suites, { id: suiteId, ...suiteMeta }]);
      else setSuites(suites.map(s => s.id === suiteId ? { ...s, ...suiteMeta } : s));
  ...
  const handleUpdateSuiteVisibility = async (suiteId, visibility) => {
    try {
      ...
      setSuites(suites.map(s => s.id === suiteId ? { ...s, ...updates } : s));
        ...
        <div className="md:col-span-2">
          <Card noPadding={true}>
            ...
            <table className="w-full text-left border-collapse">
              <tbody>
                {suites.length === 0 && (
                  <tr><td className="py-8 text-center text-xs text-on-surface-variant">Nenhuma Suite criada.</td></tr>
                )}
                
                {suites.map(suite => {
                  const suiteGroups = groups.filter(g => g.suiteId === suite.id);
                  return (
                    <React.Fragment key={suite.id}>
                        ...
                        <td className="py-2 pl-3 pr-2">
                          <div className="flex justify-between items-start">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <div className="font-bold text-on-surface text-sm">{suite.name}</div>
                                {suite.description && <div className="text-xs text-on-surface-variant mt-0.5">{suite.description}</div>}
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-3">
                              <select 
                                ...
                                onChange={e => handleUpdateSuiteVisibility(suite.id, e.target.value)}
                              >
                              ...
                              <div className="flex gap-1 opacity-0 group-hover/suite:opacity-100 transition-opacity">
                                <button className="w-6 h-6 rounded text-on-surface-variant hover:text-white hover:bg-surface-container-highest flex items-center justify-center transition-colors" aria-label="Editar Suite" title="Editar Suite" onClick={() => handleEditSuite(suite)}>
                                  <span className="material-symbols-outlined text-[14px]">edit</span>
                                </button>
                                <button className="w-6 h-6 rounded text-on-surface-variant hover:text-white hover:bg-surface-container-highest flex items-center justify-center transition-colors" aria-label="Adicionar Grupo" title="Adicionar Grupo" onClick={() => handleAddGroup(suite.id)}>
                                  <span className="material-symbols-outlined text-[14px]">create_new_folder</span>
                                </button>
                                <button className="w-6 h-6 rounded text-on-surface-variant hover:text-error hover:bg-error/10 flex items-center justify-center transition-colors" aria-label="Excluir Suite" title="Excluir Suite" onClick={() => handleDeleteSuite(suite.id)}>
                      ...
                      {suiteGroups.map(group => {
                        const groupCmdAssignments = groupAssignments.filter(a => a.groupId === group.id);
                        return (
                          <tr 
                            key={group.id} 
                              ...
                              <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1">
                                  <div className="flex justify-between items-center mb-1">
                                    ...
                                    <button className="w-5 h-5 rounded text-on-surface-variant hover:text-error hover:bg-error/10 flex items-center justify-center opacity-0 group-hover/dropzone:opacity-100 transition-opacity" aria-label="Excluir Grupo" onClick={() => handleDeleteGroup(group.id)} title="Excluir Grupo">
                                      <span className="material-symbols-outlined text-[14px]">delete</span>
                                    </button>
                                  </div>
                                    ...
                                    {groupCmdAssignments.map(assignment => {
                                      ...
                                      return (
                                        <div key={assignment.id} className="bg-surface-container-high border border-outline-variant rounded-full flex items-center gap-1.5 pr-1.5 h-6 group/chip shadow-sm relative z-10 hover:bg-surface-container-highest transition-colors cursor-default">
                                          ...
                                          <button className="w-3.5 h-3.5 flex items-center justify-center text-on-surface-variant hover:text-error opacity-0 group-hover/chip:opacity-100 transition-opacity" aria-label="Remover atribuição" onClick={() => handleRemoveAssignment(assignment.id)}>
      ...
      {showSuiteModal && (
        <div className="fixed inset-0 z-[100] bg-black/80 flex items-center justify-center p-4">
          <div className="bg-surface-container border border-outline-variant rounded-xl w-full max-w-[500px] p-6 shadow-2xl">
            <h3 className="mt-0 mb-4 text-xl text-on-surface">{editSuiteData.isNew ? 'Criar Nova Suite' : 'Editar Suite'}</h3>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button className="px-4 py-2 text-sm text-on-surface-variant hover:text-white" onClick={() => setShowSuiteModal(false)}>Cancelar</button>
              <button className="px-4 py-2 text-sm bg-primary-container text-white font-bold rounded hover:bg-[#e66000]" onClick={handleSaveSuite}>Salvar Suite</button>
            </div>
          </div>
        </div>
      )}
