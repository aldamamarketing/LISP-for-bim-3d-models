...
export default function LicensesTab() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      collection(db, "users", userData.id, "devices"),
      (snap) => {
        const devs = [];
  ...
  const handleUnlink = async (device) => {
    ...
    try {
      ...
      try {
        ...
        console.error("Error al desligar device de subcoleccion", e);
      }
  ...
  const handleDeleteDevice = async (deviceObj) => {
    if (
      !confirm(
        `¿Estás seguro de que deseas ELIMINAR permanentemente ${deviceObj.name || deviceObj.id}? Esto lo removerá de todas las suscripciones.`,
      )
    )
      return;
    try {
  ...
  const handleLink = async (deviceObj) => {
    try {
      ...
      const legacyCount = (userData.registeredDevices || []).filter(
        (d) => !devicesList.find((dev) => dev.id === d),
      ).length;
      ...
      if (totalActive >= maxSeats) {
        showToast(
          `Limite de assentos atingido (${maxSeats}). Desvincule outro primeiro.`,
          "error",
        );
        return;
      }
      ...
      if (deviceObj.globalUnlinkedAt) {
        ...
        if (daysPassed < 7) {
          ...
          showToast(
            `Anti-Abuso: Este PC só pode ser vinculado novamente em ${daysLeft} dias.`,
            "error",
          );
          return;
        }
    ...
    <div className="tab-enter grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
      ...
      <Card>
        ...
        <p className="text-sm text-on-surface-variant mb-4 mt-2">
          {t("dashboard.licenses.desc")}
        </p>

        <div className="bg-[#0D0D0D] rounded-lg p-4 mb-4 space-y-3">
          <div className="flex justify-between items-center">
            <strong>{t("dashboard.licenses.currentPlan")}</strong>{" "}
            <span className="text-green-400">
              {t("dashboard.licenses.betaTester")}
            </span>
          </div>
          <div className="flex justify-between items-center">
            <strong>{t("dashboard.licenses.basePrice")}</strong>{" "}
            <span>US$ 0.00 / mês</span>
          </div>
          <div className="flex justify-between items-center">
        ...
        <div className="mt-5 pt-4 border-t border-surface-variant">
          <h4 className="m-0 mb-3 text-sm font-bold">
            {t("dashboard.licenses.accessKey")}
          </h4>
          <div className="flex gap-3">
            ...
            <button
              ...
              onClick={() =>
                (window.location.href = `https://us-central1-lispcentral.cloudfunctions.net/generateLoader?token=${userData?.apiKey}`)
              }
            >
      ...
      <Card>
        <CardHeader
          title={`${t("dashboard.equipment.title")} (${devicesList.filter((d) => d.globalLinked).length + registeredDevices.filter((d) => !devicesList.find((dev) => dev.id === d)).length} / ${userData?.maxSeats || 1})`}
          icon="computer"
        />
        <p className="text-sm text-on-surface-variant mb-4 mt-2">
          {t("dashboard.equipment.desc")}
        </p>
        ...
        {devicesList.length > 0 || registeredDevices.length > 0 ? (
          <div className="flex flex-col gap-3">
            ...
            {devicesList.map((dev) => (
              <div
                key={dev.id}
                className={`p-3 rounded-lg border ${dev.globalLinked ? "bg-[#0D0D0D] border-surface-variant" : "bg-surface-container-low border-outline-variant/30 opacity-70"}`}
              >
                <div className="flex justify-between items-center mb-2">
                  <span
                    className={`font-mono text-sm flex items-center gap-2 ${dev.globalLinked ? "text-green-400" : "text-on-surface-variant"}`}
                  >
                ...
                <div className="text-xs text-on-surface-variant mb-2">
                  Última conexión:{" "}
                  {dev.lastActive
                    ? new Date(dev.lastActive.toDate()).toLocaleString()
                    : "N/A"}
                </div>
            ...
            {/* Fallback Legacy */}
            {registeredDevices
              .filter((d) => !devicesList.find((dev) => dev.id === d))
              .map((dev) => (
                <div
                  key={dev}
                  className="bg-[#0D0D0D] p-3 rounded-lg border border-surface-variant"
                >
                  <div className="flex justify-between items-center mb-2">
