...
export default function SupportModal() {
  ...
  const handleSendSupport = async (e) => {
    e.preventDefault();
    if (!supportMsg.trim() || isSubmitting) return;
  ...
  return (
    ...
    >
      <div className="bg-surface-container border border-surface-variant rounded-xl w-full max-w-md p-6 relative">
        ...
        <h3 id="support-modal-title" className="text-xl font-bold mb-4">
        ...
        <form onSubmit={handleSendSupport}>
          <div className="flex gap-4 mb-4">
            ...
            <div className="flex-1">
              ...
              >
                ...
                <option value="loader">
                  {t("dashboard.support.loc_loader")}
                </option>
                <option value="lisp">{t("dashboard.support.loc_lisp")}</option>
                <option value="palette">
                  {t("dashboard.support.loc_palette")}
                </option>
              </select>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full bg-primary-container text-white font-bold py-3 rounded-lg hover:bg-[#e66000] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isSubmitting ? (
              <div className="flex items-center justify-center gap-2">
                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                <span>{t("dashboard.support.send")}</span>
              </div>
            ) : (
