...
export default function LispCommandPalette() {
  ...
  const { filteredCmds, grouped, sortedGroups } = useMemo(() => {
    // ⚡ Bolt Optimization: Transform active filters once outside the loop
    const activeFiltersLower = activeFilters.map(tag => tag.toLowerCase());

    const filteredCmds = commands.filter(cmd => {
      if (activeFiltersLower.length === 0) return true;
      const searchableText = `${cmd.name || ''} ${cmd.friendly || ''} ${cmd.desc || ''} ${cmd.group || ''}`.toLowerCase();
      // Must match at least ONE tag (OR logic)
      return activeFiltersLower.some(tagLower => searchableText.includes(tagLower));
    });
    ...
    return { filteredCmds, grouped, sortedGroups };
