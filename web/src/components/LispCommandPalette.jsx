...
export default function LispCommandPalette() {
  ...
  // Filter and group
  const { filteredCmds, sortedGroups, grouped } = useMemo(() => {
    const lowercaseFilters = activeFilters.map(tag => tag.toLowerCase());

    const filtered = commands.filter(cmd => {
      if (lowercaseFilters.length === 0) return true;
      const searchableText = `${cmd.name || ''} ${cmd.friendly || ''} ${cmd.desc || ''} ${cmd.group || ''}`.toLowerCase();
      // Must match at least ONE tag (OR logic)
      return lowercaseFilters.some(tag => searchableText.includes(tag));
    });

    const groupsObj = {};
    filtered.forEach(cmd => {
      const g = cmd.group || 'Outros';
      if (!groupsObj[g]) groupsObj[g] = [];
      groupsObj[g].push(cmd);
    });

    const sorted = Object.keys(groupsObj).sort((a, b) => {
    ...
    return { filteredCmds: filtered, sortedGroups: sorted, grouped: groupsObj };
