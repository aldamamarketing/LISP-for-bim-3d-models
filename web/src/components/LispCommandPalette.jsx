...
export default function LispCommandPalette() {
  ...
  // Memoize active filters in lowercase to avoid O(N*M) string allocations
  const lowerActiveFilters = useMemo(() => {
    return activeFilters.map(tag => tag.toLowerCase());
  }, [activeFilters]);

  // Filter and group
  const { filteredCmds, grouped, sortedGroups } = useMemo(() => {
    const filtered = commands.filter(cmd => {
      if (lowerActiveFilters.length === 0) return true;
      const searchableText = `${cmd.name || ''} ${cmd.friendly || ''} ${cmd.desc || ''} ${cmd.group || ''}`.toLowerCase();
      // Must match at least ONE tag (OR logic)
      return lowerActiveFilters.some(tag => searchableText.includes(tag));
    });
