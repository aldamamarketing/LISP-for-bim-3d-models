...
export default function LispCommandPalette() {
  ...
  // ⚡ Bolt Optimization: Memoize filtering and grouping to prevent O(N*M) string allocations
  // on every render, which is critical for performance in AutoCAD's embedded browser.
  const { filteredCmds, grouped, sortedGroups } = useMemo(() => {
    ...
    filtered.forEach((cmd) => {
      ...
      if (!groupedObj[g]) groupedObj[g] = [];
      groupedObj[g].push(cmd);
    });

    const sorted = Object.keys(groupedObj).sort((a, b) => {
