...
export default function LispCommandPalette() {
  ...
  // Filter and group - Memoized to prevent O(N*M) recalculations on every render
  const { filteredCmds, grouped, sortedGroups } = useMemo(() => {
    let filtered = commands;

    if (activeFilters.length > 0) {
      // Extract expensive string transformation outside the loop
      const lowerTags = activeFilters.map((t) => t.toLowerCase());
