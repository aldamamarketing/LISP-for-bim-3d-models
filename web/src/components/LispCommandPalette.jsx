...
export default function LispCommandPalette() {
  ...
  // - Memoize filtered and grouped commands to prevent recalculation on every render
  //   (e.g., when toggling favorites or opening dropdowns).
  // - Pre-calculate .toLowerCase() on filters outside the inner loop to avoid
  //   O(N*M) string allocations during typing.
  // - Expected Impact: Instantaneous search filtering without blocking the AutoCAD UI thread.
  // Filter and group
  const { filteredCmds, grouped, sortedGroups } = useMemo(() => {
    ...
    const sGroups = Object.keys(gMap).sort((a, b) => {
    ...
    return { filteredCmds, grouped: gMap, sortedGroups: sGroups };
