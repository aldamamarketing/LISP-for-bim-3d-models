    ...
    <div className="card" style={{ margin: 0, minHeight: "600px" }}>
      <div className="flex justify-between items-center flex-wrap gap-4 mb-6">
        ...
        <div className="flex gap-4 items-center">
          ...
          />
          <a
            href="/pt/tools/hatch-generator"
            className="bg-[#1a1c1c] text-white border border-[#262626] hover:border-primary-container hover:text-primary-container px-4 py-2 rounded text-sm transition-colors flex items-center gap-2"
          >
      ...
      {loading ? (
        ...
        >
          <p style={{ color: "var(--text-muted)" }}>
            Você não possui recursos salvos nesta categoria.
          </p>
          <div
            style={{
              marginTop: "20px",
          ...
          >
            ...
            {activeTab === "lin" && (
              <a
                href="/pt/tools/linetype-generator"
                style={{
                  color: "#fff",
                  textDecoration: "none",
                  backgroundColor: "var(--tmd-orange)",
        ...
        <div style={{ display: "flex", gap: "30px", flexWrap: "wrap" }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredFavorites.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className={`rounded border transition-colors flex flex-col relative group ${isSelected ? "bg-primary-container/10 border-primary-container" : "bg-white/5 border-[#262626] hover:bg-[#1a1c1c] hover:border-[#343535]"}`}
                  >
                    <button
                      onClick={() => toggleSelection(item.id)}
                      className="w-full text-left flex-1 p-4 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-container rounded-t"
                    ...
                    >
                      <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                        <input
                          ...
                          readOnly
                          tabIndex="-1"
                          className="cursor-pointer accent-primary-container"
                        />
                      </div>
                      {isSelected && (
                      ...
                      <strong className="text-white font-bold block mb-1 pr-6 font-code-sm">
                        {item.name}
                      </strong>
                      ...
                      <div className="pointer-events-none bg-[#0A0A0A] rounded border border-[#262626] overflow-hidden p-2 flex items-center justify-center min-h-[80px]">
                        {activeTab === "hatch" && (
                          <HatchPreview patCode={item.code} scale={0.5} />
                        )}
                        {activeTab === "lin" && (
                    ...
                    <div className="p-3 border-t border-[#262626] flex justify-end">
                      <button
                        ...
                        className="text-on-surface-variant hover:text-error transition-colors flex items-center gap-1 text-xs p-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-error rounded"
                        title="Apagar"
          ...
          >
            <div className="bg-[#1A1A1A] rounded border border-[#333] shadow-2xl overflow-hidden">
              ...
              <div className="bg-[#2A2B2C] px-3 py-1.5 flex justify-between items-center border-b border-[#444]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] font-bold text-[#CCC] uppercase tracking-wider">
                    Tool Palettes - LispCentral
                  </span>
                </div>
                <div className="text-[#888] text-xs cursor-pointer hover:text-white">
                  ✖
                </div>
              </div>
              ...
              <div className="bg-[#3A3B3D] p-3 min-h-[200px] max-h-[400px] overflow-y-auto">
                {selectedItems.length === 0 ? (
                  <div className="text-[#999] text-xs text-center py-10 px-2 font-mono">
                    ...
                    <br />
                    Selecione itens ao lado para adicionar à paleta.
                  </div>
                ) : (
                  <div className="grid grid-cols-3 gap-1">
                    {selectedItems.map((item) => (
                      <div
                        key={"mock-" + item.id}
                        className="bg-[#2A2B2C] border border-[#444] h-[70px] flex flex-col items-center justify-center p-1 relative hover:border-[#666] cursor-pointer"
                        title={item.name}
                      >
                        <div className="w-full h-[40px] overflow-hidden flex items-center justify-center opacity-90 mix-blend-screen">
                          {activeTab === "hatch" && (
                            <HatchPreview patCode={item.code} scale={0.2} />
                          )}
                          {activeTab === "lin" && (
                            <LinetypePreview linCode={item.code} scale={0.5} />
                          )}
                          {activeTab === "icon" && (
                            <SvgPreview svgString={item.svgCode || item.code} />
                          )}
                        </div>
                        <span className="text-[9px] text-[#CCC] text-center mt-1 truncate w-full">
                          {item.name}
                        </span>
                      </div>
                    ))}
            ...
            >
              <h4 style={{ margin: "0 0 15px 0", color: "var(--tmd-orange)" }}>
                Baixar Selecionados
              </h4>
              ...
              <button
                ...
                style={{
                  ...
                  padding: "12px",
                  backgroundColor:
                    selectedItems.length > 0 ? "var(--tmd-orange)" : "#555",
                  color: "#fff",
                  border: "none",
                  borderRadius: "4px",
              ...
              >
                {activeTab === "icon"
                  ? `Baixar .ZIP (${selectedItems.length})`
                  : `Baixar Único Arquivo (${selectedItems.length})`}
              </button>
              ...
              >
                Com o Loader instalado, você não precisa baixar. Basta
                sincronizar sua paleta no AutoCAD.
              </div>
            </div>
          </div>
        </div>
