          ...
          <div style={{ flex: '1 1 60%' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredFavorites.map(item => {
                ...
                return (
                  <div key={item.id} className={`rounded border transition-colors flex flex-col relative group ${isSelected ? 'bg-primary-container/10 border-primary-container' : 'bg-white/5 border-[#262626] hover:bg-[#1a1c1c] hover:border-[#343535]'}`}>
                    
                    {/* Native accessible selection button covering the card */}
                    <button
                      onClick={() => toggleSelection(item.id)}
                      aria-label={`Selecionar ${item.name}`}
                      aria-pressed={isSelected}
                      className="absolute inset-0 w-full h-full cursor-pointer bg-transparent border-none p-0 m-0 z-0 focus-visible:ring-2 focus-visible:ring-primary-container focus:outline-none"
                    />

                    <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity z-10 pointer-events-none">
                      <input type="checkbox" checked={isSelected} readOnly tabIndex={-1} className="cursor-pointer accent-primary-container" />
                    </div>
                    {(isSelected) && (
                    ...
                    <div className="p-3 border-t border-[#262626] flex justify-end z-10 relative">
                      <button 
                        onClick={(e) => { e.stopPropagation(); handleRemove(item.id); }}
                        aria-label={`Apagar ${item.name}`}
                        className="text-on-surface-variant hover:text-error transition-colors flex items-center gap-1 text-xs focus-visible:ring-2 focus-visible:ring-error focus:outline-none rounded"
                        title="Apagar"
                      >
