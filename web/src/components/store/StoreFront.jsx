...
const SuiteRow = ({ suite, currentUser, initiallyExpanded }) => {
  ...
  const handleToggle = async (e) => {
    ...
    if (willExpand && commands === null) {
      ...
      try {
        ...
        const fetchedGroups = gSnap.docs.map((d) => ({
          id: d.id,
          ...d.data(),
        }));
        ...
        if (fetchedGroups.length > 0) {
          ...
          for (const chunk of chunks) {
            ...
            allAssignments = [
              ...allAssignments,
              ...gcSnap.docs.map((d) => ({ id: d.id, ...d.data() })),
            ];
          }
          ...
          if (allAssignments.length > 0) {
            ...
            for (let i = 0; i < fileIds.length; i += 10) {
              fileChunks.push(fileIds.slice(i, i + 10));
            }

            for (const chunk of fileChunks) {
              const cQ = query(
                collection(db, "commands"),
                where("lispFileId", "in", chunk),
              );
  ...
  return (
    <div
      className={`bg-surface-container-low border ${expanded ? "border-primary-container/50" : "border-outline-variant"} rounded-md overflow-hidden transition-all duration-300 hover:border-outline`}
    >
      ...
      >
        ...
        <div className="ml-4 flex-1 min-w-0 flex items-center gap-4">
          <div className="font-bold text-on-surface text-sm truncate w-48 shrink-0">
            {suite.name}
          </div>
          ...
          <div className="w-24 shrink-0 flex items-center text-xs text-on-surface-variant">
            <span
              className="material-symbols-outlined text-[14px] text-primary-container mr-1"
              style={{ fontVariationSettings: "'FILL' 1" }}
            >
              star
            </span>
            {suite.rating ? suite.rating.toFixed(1) : "N/A"} (
            {suite.ratingCount || 0})
          </div>
        ...
        <div className="ml-4 flex items-center gap-3 shrink-0">
          <button
            ...
            onClick={
              hasSubscribed ? (e) => e.stopPropagation() : handleSubscribe
            }
          ...
          >
            {isSubscribing
              ? "..."
              : hasSubscribed
                ? t("store.subscribed", "Suscrito")
                : t("store.free_beta", "Gratis (Beta)")}
          </button>

          <button className="text-on-surface-variant hover:text-on-surface w-6 flex justify-center transition-transform">
            <span
              className={`material-symbols-outlined text-[20px] transform transition-transform duration-300 ${expanded ? "rotate-180 text-primary-container" : ""}`}
            >
              expand_more
            </span>
          </button>
        ...
        <div className="border-t border-outline-variant bg-surface-container-lowest p-5 animate-in slide-in-from-top-2 fade-in duration-300">
          ...
          <div className="mb-5 flex justify-between items-start">
            <div>
              <h4 className="text-xs font-bold text-on-surface uppercase tracking-wider mb-2">
                {t("store.about_suite", "Acerca de esta Suite")}
              </h4>
              <p className="text-sm text-on-surface-variant max-w-3xl leading-relaxed">
                {suite.description ||
                  t(
                    "store.no_desc",
                    "Esta suite no posee una descripción extendida. Comuníquese con el desarrollador para más detalles.",
                  )}
              </p>
            </div>
            {hasSubscribed && (
          ...
          {reviewModalOpen && (
            <div className="mb-5 p-4 bg-surface border border-primary-container/30 rounded-md max-w-xl animate-in fade-in zoom-in-95">
              <h5 className="text-sm font-bold text-on-surface mb-3">
                {t("store.leave_review", "Deja tu valoración para")}{" "}
                {reviewTarget.name}
              </h5>
              ...
              <textarea
                ...
                rows="2"
                placeholder={t(
                  "store.review_placeholder",
                  "Cuenta tu experiencia (opcional)...",
                )}
                value={reviewComment}
              ...
              <div className="flex justify-end gap-2">
                ...
                >
                  {isSubmittingReview
                    ? t("store.sending", "Enviando...")
                    : t("store.send_review", "Enviar Valoración")}
                </button>
              </div>
            </div>
          )}
            ...
            {loadingDetails ? (
              ...
              <div className="max-h-64 overflow-y-auto custom-scrollbar">
                <table className="w-full text-left text-sm border-collapse">
                  <thead className="bg-surface-container-lowest sticky top-0 z-10 border-b border-outline-variant/30">
                    <tr>
                      <th className="py-1.5 px-2 font-code-sm text-[10px] text-on-surface-variant uppercase w-10">
                        {t("store.icon", "Ícono")}
                      </th>
                      <th className="py-1.5 px-2 font-code-sm text-[10px] text-on-surface-variant uppercase w-48">
                        {t("store.command", "Comando")}
                      </th>
                      <th className="py-1.5 px-2 font-code-sm text-[10px] text-on-surface-variant uppercase">
                        {t("store.description", "Descripción")}
                      </th>
                  ...
                  <tbody className="divide-y divide-outline-variant/10">
                    {commands.map((cmd) => (
                      <tr
                        key={cmd.id}
                        className="hover:bg-surface-container-high transition-colors group/row"
                      >
                        ...
                        <td className="py-1.5 px-2">
                          <div className="font-bold text-on-surface text-xs">
                            {cmd.friendlyName || cmd.commandName}
                          </div>
                          <div className="text-[10px] text-primary-container font-code-sm opacity-60">
                            {cmd.commandName}
                          </div>
                        </td>
              ...
              <div className="p-6 flex flex-col items-center justify-center text-on-surface-variant opacity-70">
                ...
                <span className="text-sm">
                  {t(
                    "store.no_commands",
                    "Ningún comando encontrado en esta suite.",
                  )}
                </span>
              </div>
            )}
...
export default function StoreFront() {
  ...
  useEffect(() => {
    const fetchSuites = async () => {
      try {
        ...
        const fetchedSuites = snap.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
  ...
  const filteredSuites = useMemo(() => {
    // ⚡ Bolt: Extract expensive string transformation outside the loop (O(N) improvement)
    ...
    return suites.filter((suite) => {
      const matchesSearch =
        ...
        suite.authorName?.toLowerCase().includes(searchLower);

      const matchesCategory =
        !filterCategory || suite.storeCategory === filterCategory;
      const matchesPlatform =
        !filterPlatform || suite.compatibility === filterPlatform;
      const matchesVersion =
        !filterVersion || suite.supportedVersions === filterVersion;

      return (
        matchesSearch && matchesCategory && matchesPlatform && matchesVersion
      );
    });
  }, [suites, searchQuery, filterCategory, filterPlatform, filterVersion]);
    ...
    <div className="max-w-6xl mx-auto pb-12">
      ...
      <div className="mb-6 border-b border-outline-variant pb-4">
        <div className="flex justify-between items-end mb-4">
          <div>
            <h1 className="text-2xl font-bold text-on-surface">
              {t("store.marketplace_title", "Extension Marketplace")}
            </h1>
            <p className="text-sm text-on-surface-variant mt-1">
              {t(
                "store.marketplace_subtitle",
                "Busque y suscriba suites de herramientas para su flujo de trabajo.",
              )}
            </p>
          </div>
          <div className="relative">
            ...
            />
            <span className="material-symbols-outlined text-[16px] absolute left-2.5 top-2 text-on-surface-variant">
              search
            </span>
          </div>
        </div>
        ...
        <div className="flex gap-3">
          ...
          >
            <option value="">
              {t("store.all_categories", "Todas las Categorías")}
            </option>
            {/* We dynamically extract categories from the loaded suites, plus defaults */}
            {[
              ...new Set([
                ...suites.map((s) => s.storeCategory).filter(Boolean),
                "architecture",
                "civil",
                "topo",
                "structure",
          ...
          >
            <option value="">
              {t("store.platform_any", "Plataforma (Cualquiera)")}
            </option>
          ...
          >
            <option value="">
              {t("store.version_any", "Versión (Cualquiera)")}
            </option>
      ...
      <div className="flex flex-col gap-2">
        {filteredSuites.length === 0 ? (
          <div className="py-12 text-center text-on-surface-variant italic">
            {t(
              "store.no_results",
              "No se encontraron suites con estos filtros.",
            )}
          </div>
        ) : (
