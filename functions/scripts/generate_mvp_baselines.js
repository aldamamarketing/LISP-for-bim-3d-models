...
try {
  ...
  console.error("Falta el archivo 'serviceAccountKey.json' en la carpeta functions.");
  process.exit(1);
}
...
async function generateAndUpload() {
  ...
  for (const archetypeId of archetypes) {
    ...
    try {
      ...
      count++;
      console.log(`[OK] Generado MVP: ${safeId}`);
  ...
  await batch.commit();
  console.log(`\n¡Éxito! ${count} MVPs subidos a la base de datos Firestore.`);
}
