...
## 6. Actualizaci�n Reciente y Pendientes (Junio 2026)
...
1.  **Despliegue del Backend (Troubleshooting)**
    *   Solucionar el Timeout de Firebase CLI (Cannot determine backend specification) al hacer deploy de unctions. La l�gica ya est� en c�digo fuente pero necesita desplegarse a producci�n exitosamente.
2.  **Verificaci�n de Aparejos Complejos en AutoCAD**
    *   Patrones hist�ricos complejos (Gothic Bond, Flemish) que utilizan l�neas discontinuas en el Eje Y deben ser insertados en AutoCAD y probados visualmente para asegurar que no saturen el l�mite de MaxHatch.
3.  **Puente AutoLISP ? UI (Integraci�n Final)**
    *   Asegurar que el Base64 viaje v�a window.chrome.webview.postMessage y sea inyectado en AutoCAD por LC_APPLY_ASSET directamente.
